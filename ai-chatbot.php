<?php
/**
 * AI Chatbot - ตอบคำถามอัตโนมัติด้วย OpenAI GPT
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: ai-chatbot.php?saved=1');
    exit;
}
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <form method="POST" class="space-y-4">
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_enabled" class="sr-only peer" <?= $settings['is_enabled'] ? 'checked' : '' ?>>
            ...
            <div>
                ...
                <textarea name="system_prompt" rows="6" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="กำหนดบุคลิกและพฤติกรรมของ AI..."><?= htmlspecialchars($settings['system_prompt']) ?></textarea>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="max_tokens" value="<?= $settings['max_tokens'] ?>" min="50" max="4000" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1">Temperature</label>
                    <input type="number" name="temperature" value="<?= $settings['temperature'] ?>" min="0" max="2" step="0.1" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                </div>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <div class="space-y-3">
                <div class="p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100" onclick="setPrompt(this.querySelector('p').textContent)">
