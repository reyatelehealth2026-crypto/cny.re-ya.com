...
try {
    switch ($action) {
        case 'send_message':
            ...
            if (!$userId || !$message) throw new Exception('Missing required fields');
            ...
            if ($result['code'] !== 200) throw new Exception('Failed to send message');
            ...
            echo json_encode(['success' => true, 'message_id' => $db->lastInsertId(), 'content' => $message, 'time' => date('H:i'), 'method' => $result['method'] ?? 'push']);
            break;
        ...
        case 'get_messages':
            ...
            if ($lastId > 0) { $sql .= " AND id > ?"; $params[] = $lastId; }
        ...
        case 'assign_tag':
            ...
            if (!$userId || !$tagId) throw new Exception('Missing required fields');
            ...
            $stmt->execute([$tagId]);
            echo json_encode(['success' => true, 'tag' => $stmt->fetch(PDO::FETCH_ASSOC), 'message' => 'เพิ่ม Tag สำเร็จ']);
            break;
        ...
        case 'create_tag':
            ...
            if (!$name) throw new Exception('กรุณาระบุชื่อ Tag');
        ...
        case 'update_tag':
            ...
            $color = $_POST['color'] ?? '#3B82F6';
            $description = trim($_POST['description'] ?? '');
            if (!$tagId || !$name) throw new Exception('กรุณาระบุข้อมูลให้ครบ');
        ...
        case 'update_order_status':
            ...
            if (!$orderId || !$newStatus) throw new Exception('Missing required fields');
        ...
        case 'approve_slip':
            ...
            if (!$orderId || !$slipId) throw new Exception('Missing required fields');
            $stmt = $db->prepare("SELECT o.*, u.line_user_id, u.reply_token, u.reply_token_expires FROM transactions o LEFT JOIN users u ON o.user_id = u.id WHERE o.id = ? AND (o.line_account_id = ? OR o.line_account_id IS NULL)");
            $stmt->execute([$orderId, $currentBotId]);
            $order = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$order) throw new Exception('Order not found');
        ...
        case 'upload_product_image':
            if (empty($_FILES['image']['name'])) throw new Exception('No image uploaded');
            $uploadDir = __DIR__ . '/../uploads/products/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
            $ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
            $allowed = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
            if (!in_array($ext, $allowed)) throw new Exception('ไฟล์รูปภาพไม่ถูกต้อง');
        ...
        case 'update_category':
            ...
            try { $db->query("SELECT 1 FROM item_categories LIMIT 1"); $catTable = 'item_categories'; } catch (Exception $e) {}
        ...
        case 'update_auto_reply':
            ...
            $isActive = isset($_POST['is_active']) ? 1 : 0;
            if (empty($keyword) || empty($replyContent)) throw new Exception('กรุณากรอกข้อมูลให้ครบ');
        ...
        case 'delete_auto_reply':
            $id = (int)$_POST['id'];
            if (!$id) throw new Exception('Missing rule id');
