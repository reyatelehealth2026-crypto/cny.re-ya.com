<?php
/**
 * Consent Management - Admin Page
 * จัดการและดู Audit Log ความยินยอม
 */
...
try {
    ...
    $stats['total_consented'] = $stmt->fetchColumn();
...
<div class="space-y-6">
    ...
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
        ...
        <p class="text-sm mt-2">กรุณารัน migration ก่อน: <a href="run_consent_migration.php" class="underline">run_consent_migration.php</a></p>
    </div>
    <?php else: ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl shadow p-4">
            <div class="text-3xl font-bold text-blue-600"><?= number_format($stats['total_consented'] ?? 0) ?></div>
            <div class="text-gray-500 text-sm">ผู้ใช้ที่ยินยอมแล้ว</div>
        </div>
        <div class="bg-white rounded-xl shadow p-4">
            <div class="text-3xl font-bold text-green-600"><?= number_format($stats['by_type']['privacy_policy'] ?? 0) ?></div>
            <div class="text-gray-500 text-sm">ยอมรับ Privacy Policy</div>
        </div>
        <div class="bg-white rounded-xl shadow p-4">
            <div class="text-3xl font-bold text-purple-600"><?= number_format($stats['by_type']['terms_of_service'] ?? 0) ?></div>
            <div class="text-gray-500 text-sm">ยอมรับ Terms of Service</div>
        </div>
        <div class="bg-white rounded-xl shadow p-4">
            <div class="text-3xl font-bold text-orange-600"><?= number_format($stats['by_type']['health_data'] ?? 0) ?></div>
    ...
    <div class="bg-white rounded-xl shadow">
        <div class="border-b">
            <nav class="flex -mb-px">
                <button onclick="showTab('consent')" id="tab-consent" class="tab-btn px-6 py-3 border-b-2 border-blue-500 text-blue-600 font-medium">
                    📝 Consent Logs
                </button>
                <button onclick="showTab('access')" id="tab-access" class="tab-btn px-6 py-3 border-b-2 border-transparent text-gray-500 hover:text-gray-700">
                    👁️ Data Access Logs
                </button>
            </nav>
        </div>
        ...
        <!-- Consent Logs Tab -->
        <div id="panel-consent" class="p-4">
        ...
        <div id="panel-access" class="p-4 hidden">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
