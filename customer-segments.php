    ...
    <div class="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden">
        <div class="p-5">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="px-2 py-1 text-xs rounded-full <?= $segment['segment_type'] === 'dynamic' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700' ?>">
            ...
            <div class="flex gap-2">
                <a href="broadcast.php?segment=<?= $segment['id'] ?>" class="flex-1 py-2 text-center bg-green-500 text-white rounded-lg hover:bg-green-600 text-sm">
                    <i class="fas fa-paper-plane mr-1"></i>Broadcast
                </a>
                <form method="POST" class="flex-1">
                    <input type="hidden" name="action" value="recalculate">
                    <input type="hidden" name="segment_id" value="<?= $segment['id'] ?>">
                    <button type="submit" class="w-full py-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-50 text-sm">
                ...
                <form method="POST" onsubmit="return confirm('ลบ Segment นี้?')">
                    ...
                    <input type="hidden" name="segment_id" value="<?= $segment['id'] ?>">
                    <button type="submit" class="py-2 px-3 border border-red-300 text-red-500 rounded-lg hover:bg-red-50">
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 my-8">
        ...
        <form method="POST" id="createSegmentForm" class="p-6">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="เช่น High Value Customers">
            </div>
            
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">คำอธิบาย</label>
                <textarea name="description" rows="2" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500"></textarea>
            </div>
            
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2">เลือกจาก Template</label>
                <div class="grid grid-cols-2 gap-2">
                    <button type="button" onclick="applyTemplate('active')" class="p-3 border rounded-lg hover:bg-gray-50 text-left">
                        <span class="font-medium">🟢 Active Users</span>
                        <p class="text-xs text-gray-500">มี activity ใน 7 วัน</p>
                    </button>
                    <button type="button" onclick="applyTemplate('high_value')" class="p-3 border rounded-lg hover:bg-gray-50 text-left">
                        <span class="font-medium">💎 High Value</span>
                        <p class="text-xs text-gray-500">ยอดซื้อ 5,000+ บาท</p>
                    </button>
                    <button type="button" onclick="applyTemplate('at_risk')" class="p-3 border rounded-lg hover:bg-gray-50 text-left">
                        <span class="font-medium">⚠️ At Risk</span>
                        <p class="text-xs text-gray-500">ไม่มี activity 14-30 วัน</p>
                    </button>
                    <button type="button" onclick="applyTemplate('new_users')" class="p-3 border rounded-lg hover:bg-gray-50 text-left">
                        <span class="font-medium">🆕 New Users</span>
                        <p class="text-xs text-gray-500">สมัครใน 7 วัน</p>
                    </button>
                </div>
            </div>
...
function applyTemplate(type) {
    const templates = {
        ...
        'high_value': {name: 'High Value Customers', conditions: {'lifetime_value': {'>=': 5000}}},
        'at_risk': {name: 'At Risk Customers', conditions: {'last_activity_days': {'>=': 14, '<=': 30}}},
        'new_users': {name: 'New Users This Week', conditions: {'created_days': {'<=': 7}}}
    };
...
document.getElementById('createSegmentForm').addEventListener('submit', function(e) {
    if (!document.getElementById('conditionsJson').value) {
        document.getElementById('conditionsJson').value = '{"created_days": {"<=": 30}}';
    }
});
</script>
