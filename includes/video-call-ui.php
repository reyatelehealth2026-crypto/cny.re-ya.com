...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .video-full { position:fixed; inset:0; object-fit:cover; }
        .pip { position:fixed; top:1rem; right:1rem; width:120px; aspect-ratio:9/16; 
               border-radius:1rem; overflow:hidden; border:3px solid rgba(255,255,255,0.3);
               background:#1f2937; z-index:10; }
        .control-bar { position:fixed; bottom:0; left:0; right:0; padding:1.5rem;
                       background:linear-gradient(transparent, rgba(0,0,0,0.8)); z-index:20; }
        .ctrl-btn { width:60px; height:60px; border-radius:50%; display:flex; 
...
<div id="preCall" class="fixed inset-0 hidden">
    ...
    <div class="fixed top-4 left-4 right-4 z-10">
        <div class="bg-white/95 backdrop-blur rounded-2xl p-4 shadow-lg">
            <div class="flex items-center gap-3">
                ...
                <img src="<?= htmlspecialchars($appointmentInfo['pharmacist_image']) ?>" class="w-14 h-14 rounded-full object-cover border-2 border-green-500">
                <?php else: ?>
                <div class="w-14 h-14 rounded-full bg-green-100 flex items-center justify-center text-2xl">👨‍⚕️</div>
                <?php endif; ?>
                <div class="flex-1">
                    <p class="text-xs text-green-600 font-medium">นัดหมาย #<?= htmlspecialchars($appointmentInfo['appointment_id']) ?></p>
                    <p class="font-bold text-gray-800"><?= htmlspecialchars($appointmentInfo['pharmacist_name']) ?></p>
                    <p class="text-xs text-gray-500"><?= htmlspecialchars($appointmentInfo['pharmacist_title'] ?: 'เภสัชกร') ?></p>
                </div>
                <div class="text-right">
                    <p class="text-sm font-bold text-gray-800"><?= date('H:i', strtotime($appointmentInfo['appointment_time'])) ?> น.</p>
                    <p class="text-xs text-gray-500"><?= date('d/m/Y', strtotime($appointmentInfo['appointment_date'])) ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="fixed top-4 left-4 right-4 z-10">
        <div class="bg-white/95 backdrop-blur rounded-2xl p-4 shadow-lg">
            <div class="flex items-center gap-3">
                ...
                <img src="<?= htmlspecialchars($pharmacistInfo['image_url']) ?>" class="w-14 h-14 rounded-full object-cover border-2 border-teal-500">
                <?php else: ?>
                <div class="w-14 h-14 rounded-full bg-teal-100 flex items-center justify-center text-2xl">👨‍⚕️</div>
                <?php endif; ?>
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500"><?= htmlspecialchars($pharmacistInfo['title'] ?: $pharmacistInfo['specialty'] ?: 'เภสัชกร') ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="fixed top-4 right-4 flex gap-2 z-10">
        <button onclick="switchCamera()" class="w-12 h-12 bg-black/50 backdrop-blur rounded-full flex items-center justify-center text-white text-xl">🔄</button>
    </div>
</div>
...
<div id="inCall" class="fixed inset-0 hidden">
    ...
    <div class="fixed top-4 left-4 z-10">
        <div id="callStatus" class="bg-black/60 backdrop-blur px-4 py-2 rounded-full text-white flex items-center gap-2">
    ...
    <div class="control-bar">
        <div class="flex justify-center items-center gap-4">
            <button onclick="toggleMute()" id="btnMute" class="ctrl-btn bg-white/20 backdrop-blur text-white">🎤</button>
            <button onclick="endCall()" class="ctrl-btn bg-red-500 text-white w-16 h-16 text-2xl">📵</button>
            <button onclick="toggleVideo()" id="btnVideo" class="ctrl-btn bg-white/20 backdrop-blur text-white">📹</button>
...
<div id="endScreen" class="fixed inset-0 bg-gray-900 hidden flex-col items-center justify-center text-white p-6">
    ...
    <button onclick="resetCall()" class="w-full max-w-xs py-4 bg-green-500 rounded-2xl font-bold mb-3">โทรอีกครั้ง</button>
...
const API = BASE_URL + 'liff-video-call-pro.php';
