    ...
    foreach ($columns as $col) {
        ...
        if ($stmt->rowCount() == 0) {
            switch ($col) {
                ...
                case 'sender_name': $db->exec("ALTER TABLE ai_settings ADD COLUMN sender_name VARCHAR(100)"); break;
                case 'sender_icon': $db->exec("ALTER TABLE ai_settings ADD COLUMN sender_icon VARCHAR(500)"); break;
            }
        }
    }
} catch (Exception $e) {}
    ...
    try {
        ...
        if ($existing) {
            $stmt = $db->prepare("UPDATE ai_settings SET 
                ...
                WHERE id=?");
            $stmt->execute([
                $data['is_enabled'], $data['system_prompt'], $data['model'], $data['gemini_api_key'], $data['ai_mode'],
                $data['business_info'], $data['product_knowledge'], $data['sales_prompt'],
                $data['auto_load_products'], $data['product_load_limit'], $data['sender_name'], $data['sender_icon'],
                $existing['id']
            ]);
        } else {
            $stmt = $db->prepare("INSERT INTO ai_settings 
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->execute([
                $currentBotId, $data['is_enabled'], $data['system_prompt'], $data['model'], $data['gemini_api_key'], $data['ai_mode'],
                $data['business_info'], $data['product_knowledge'], $data['sales_prompt'],
                $data['auto_load_products'], $data['product_load_limit'], $data['sender_name'], $data['sender_icon']
            ]);
        }
...
.settings-btn-primary { background: #10b981; color: #fff; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
...
.settings-input-field:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }
...
.settings-toggle input:checked + .settings-toggle-slider { background: #10b981; }
...
.mode-card { padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; cursor: pointer; transition: all 0.2s; }
.mode-card:hover { border-color: #10b981; }
.mode-card.active { border-color: #10b981; background: #f0fdf4; }
</style>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="settings-card">
                <div class="settings-card-header">
                    <h3 class="settings-section-title"><i class="fas fa-key text-yellow-500"></i>ตั้งค่า Gemini API</h3>
                </div>
                <div class="settings-card-body space-y-4">
                    ...
                    <div>
                        ...
                        <p class="settings-hint"><a href="https://aistudio.google.com/app/apikey" target="_blank" class="text-blue-500 hover:underline"><i class="fas fa-external-link-alt mr-1"></i>รับ API Key ฟรี</a></p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="ai_mode" id="aiModeSelect" class="settings-input-field" onchange="toggleSalesSection()">
            ...
            <div class="settings-card">
                ...
                <div class="settings-card-body space-y-4">
                    <div>
                        ...
                        <textarea name="business_info" rows="3" class="settings-input-field" placeholder="ชื่อร้าน: ...&#10;ที่อยู่: ...&#10;เวลาทำการ: ..."><?= htmlspecialchars($businessInfo) ?></textarea>
                    </div>
                    <div>
                        <label class="settings-label">System Prompt (บทบาทหลัก)</label>
                        <textarea name="ai_system_prompt" rows="3" class="settings-input-field" placeholder="คุณคือพนักงานขายของร้าน..."><?= htmlspecialchars($systemPrompt) ?></textarea>
                        <div class="mt-2 flex flex-wrap gap-2">
                            <button type="button" onclick="setPromptTemplate('shop')" class="settings-template-btn">🛒 ร้านค้า</button>
                            <button type="button" onclick="setPromptTemplate('pharmacy')" class="settings-template-btn">💊 ร้านยา</button>
                            <button type="button" onclick="setPromptTemplate('restaurant')" class="settings-template-btn">🍜 ร้านอาหาร</button>
                            <button type="button" onclick="setPromptTemplate('support')" class="settings-template-btn">📞 Support</button>
                        </div>
                    </div>
                    <div id="salesSection" class="<?= $aiMode !== 'sales' ? 'hidden' : '' ?>">
                        <label class="settings-label">คำแนะนำสำหรับการขาย</label>
                        <textarea name="sales_prompt" rows="2" class="settings-input-field" placeholder="เน้นแนะนำโปรโมชั่น..."><?= htmlspecialchars($salesPrompt) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="settings-card">
                ...
                <div class="settings-card-body space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="product_knowledge" rows="3" class="settings-input-field font-mono text-sm" placeholder="โปรโมชั่นพิเศษ:&#10;- สินค้า A ลด 20%&#10;- ส่งฟรีเมื่อซื้อครบ 500 บาท"><?= htmlspecialchars($productKnowledge) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="settings-card">
                <div class="settings-card-header">
                    <h3 class="settings-section-title"><i class="fas fa-user-circle text-purple-500"></i>ตั้งค่าผู้ส่ง (Sender)</h3>
                </div>
                <div class="settings-card-body">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="settings-label">ชื่อผู้ส่ง</label>
                            <input type="text" name="sender_name" value="<?= htmlspecialchars($senderName) ?>" class="settings-input-field" placeholder="เช่น: AI Assistant">
                        </div>
                        <div>
                            <label class="settings-label">Icon URL</label>
                            <input type="url" name="sender_icon" value="<?= htmlspecialchars($senderIcon) ?>" class="settings-input-field" placeholder="https://...">
                        </div>
                    </div>
        ...
        <div class="space-y-5">
            ...
            <div class="settings-card">
                <div class="settings-card-body">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between"><span class="text-gray-500">API Key:</span><span class="<?= $apiKey ? 'text-green-600' : 'text-red-600' ?>"><?= $apiKey ? '✅ ตั้งค่าแล้ว' : '❌ ยังไม่ได้ตั้งค่า' ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">สถานะ:</span><span><?= $isEnabled ? '🟢 เปิด' : '⚪ ปิด' ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">โหมด:</span><span class="px-2 py-1 rounded text-xs <?= $aiMode === 'sales' ? 'bg-emerald-100 text-emerald-700' : ($aiMode === 'pharmacist' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100') ?>"><?= $aiMode === 'sales' ? '🛒 พนักงานขาย' : ($aiMode === 'pharmacist' ? '💊 เภสัชกร' : '💬 ซัพพอร์ต') ?></span></div>
            ...
            <div class="settings-card">
                ...
                <div id="settingsChatMessages" class="h-48 overflow-y-auto p-4 space-y-3 bg-slate-50 border-b border-slate-100">
                    <div class="text-center text-slate-400 text-sm">พิมพ์ข้อความเพื่อทดสอบ AI</div>
                </div>
                <div class="p-3 flex gap-2">
                    <input type="text" id="settingsTestMessage" placeholder="พิมพ์ทดสอบ..." class="settings-input-field flex-1" onkeypress="if(event.key==='Enter'){event.preventDefault();sendTestMessage();}">
                    <button type="button" onclick="sendTestMessage()" class="settings-btn-primary px-4"><i class="fas fa-paper-plane"></i></button>
                </div>
            </div>
            ...
            <div class="settings-card">
                <div class="settings-card-body">
                    <h4 class="font-semibold mb-3"><i class="fas fa-lightbulb text-yellow-500 mr-2"></i>เคล็ดลับ</h4>
                    <ul class="text-sm text-slate-600 space-y-2">
                        <li class="flex items-start gap-2"><i class="fas fa-check text-emerald-500 mt-0.5 text-xs"></i><span>ใส่ข้อมูลธุรกิจให้ครบ AI จะตอบได้แม่นยำขึ้น</span></li>
                        <li class="flex items-start gap-2"><i class="fas fa-check text-emerald-500 mt-0.5 text-xs"></i><span>เปิดโหลดสินค้าอัตโนมัติเพื่อให้ AI รู้จักสินค้า</span></li>
                        <li class="flex items-start gap-2"><i class="fas fa-check text-emerald-500 mt-0.5 text-xs"></i><span>ทดสอบแชทก่อนเปิดใช้งานจริง</span></li>
                    </ul>
                </div>
            </div>
        </div>
