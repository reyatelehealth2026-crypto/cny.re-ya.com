...
<div class="bg-white rounded-xl shadow mb-6">
    <div class="flex border-b overflow-x-auto">
        <a href="?tab=settings&settings_tab=rules" class="px-6 py-3 font-medium whitespace-nowrap <?= $settingsTab === 'rules' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700' ?>"><i class="fas fa-sliders-h mr-2"></i>กฎพื้นฐาน</a>
        <a href="?tab=settings&settings_tab=campaigns" class="px-6 py-3 font-medium whitespace-nowrap <?= $settingsTab === 'campaigns' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700' ?>"><i class="fas fa-bullhorn mr-2"></i>แคมเปญโบนัส<?php $activeCampaigns = array_filter($campaigns, fn($c) => $c['status'] === 'active'); if (count($activeCampaigns) > 0): ?><span class="ml-1 px-2 py-0.5 bg-green-500 text-white text-xs rounded-full"><?= count($activeCampaigns) ?></span><?php endif; ?></a>
        <a href="?tab=settings&settings_tab=categories" class="px-6 py-3 font-medium whitespace-nowrap <?= $settingsTab === 'categories' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700' ?>"><i class="fas fa-tags mr-2"></i>โบนัสหมวดหมู่</a>
        <a href="?tab=settings&settings_tab=tiers" class="px-6 py-3 font-medium whitespace-nowrap <?= $settingsTab === 'tiers' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-500 hover:text-gray-700' ?>"><i class="fas fa-medal mr-2"></i>ระดับสมาชิก</a>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-600">
                                    <span class="flex items-center gap-1"><i class="fas fa-times text-purple-500"></i><strong class="text-purple-600"><?= number_format($campaign['multiplier'], 1) ?>x</strong> แต้ม</span>
                                    <span class="flex items-center gap-1"><i class="fas fa-calendar text-gray-400"></i><?= date('d/m/Y', strtotime($campaign['start_date'])) ?> - <?= date('d/m/Y', strtotime($campaign['end_date'])) ?></span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <button type="button" onclick="toggleCampaign(<?= $campaign['id'] ?>)" class="p-2 rounded-lg hover:bg-gray-100 transition-colors" title="<?= $campaign['is_active'] ? 'ปิดใช้งาน' : 'เปิดใช้งาน' ?>">
                                    <i class="fas fa-<?= $campaign['is_active'] ? 'toggle-on text-green-500' : 'toggle-off text-gray-400' ?> text-xl"></i>
                                </button>
                                <button type="button" onclick="editCampaign(<?= htmlspecialchars(json_encode($campaign)) ?>)" class="p-2 rounded-lg hover:bg-gray-100 transition-colors text-blue-600" title="แก้ไข"><i class="fas fa-edit"></i></button>
                                <button type="button" onclick="deleteCampaign(<?= $campaign['id'] ?>, '<?= htmlspecialchars($campaign['name']) ?>')" class="p-2 rounded-lg hover:bg-red-50 transition-colors text-red-600" title="ลบ"><i class="fas fa-trash"></i></button>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow p-4">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg"><span class="text-sm text-gray-600">กำลังใช้งาน</span><span class="font-bold text-green-600"><?= count(array_filter($campaigns, fn($c) => $c['status'] === 'active')) ?></span></div>
                <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg"><span class="text-sm text-gray-600">เร็วๆ นี้</span><span class="font-bold text-blue-600"><?= count(array_filter($campaigns, fn($c) => $c['status'] === 'upcoming')) ?></span></div>
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg"><span class="text-sm text-gray-600">หมดอายุ/ปิด</span><span class="font-bold text-gray-600"><?= count(array_filter($campaigns, fn($c) => in_array($c['status'], ['expired', 'disabled']))) ?></span></div>
            </div>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow">
            ...
            <div class="p-4">
                ...
                <div class="text-center py-12">
                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4"><i class="fas fa-tags text-gray-400 text-2xl"></i></div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="border rounded-lg p-4 hover:shadow-md transition-shadow flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><i class="fas fa-tag text-green-600"></i></div>
                            <div>
                        ...
                        <div class="flex items-center gap-4">
                            <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full font-semibold"><?= number_format($bonus['multiplier'], 1) ?>x แต้ม</span>
                            <button type="button" onclick="deleteCategoryBonus(<?= $bonus['id'] ?>, '<?= htmlspecialchars($bonus['category_name'] ?? '') ?>')" class="p-2 rounded-lg hover:bg-red-50 transition-colors text-red-600" title="ลบ"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow">
            <div class="p-4 border-b"><h3 class="font-semibold text-gray-800"><i class="fas fa-plus-circle mr-2 text-purple-500"></i>เพิ่มโบนัสหมวดหมู่</h3></div>
            <form id="categoryBonusForm" class="p-4">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="category_id" id="categorySelect" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                            <option value="">-- เลือกหมวดหมู่ --</option>
                            <?php foreach ($categories as $cat): $alreadyAdded = in_array($cat['id'], array_column($categoryBonuses, 'category_id')); ?>
                            <option value="<?= $cat['id'] ?>" <?= $alreadyAdded ? 'disabled' : '' ?>><?= htmlspecialchars($cat['name']) ?><?= $alreadyAdded ? ' (มีโบนัสแล้ว)' : '' ?></option>
                            <?php endforeach; ?>
                        </select>
                        <?php if (empty($categories)): ?><p class="text-xs text-orange-500 mt-1"><i class="fas fa-exclamation-triangle mr-1"></i>ไม่พบหมวดหมู่สินค้า</p><?php endif; ?>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">ตัวคูณแต้ม <span class="text-red-500">*</span></label>
                        <div class="flex items-center gap-2">
                            <input type="number" name="multiplier" id="categoryMultiplier" value="1.5" required min="1.1" max="10" step="0.1" class="w-24 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                            <span class="text-gray-600">x แต้ม</span>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">เช่น 1.5x = ได้แต้มเพิ่ม 50%</p>
                    </div>
                </div>
                <button type="submit" class="w-full mt-4 px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors"><i class="fas fa-plus mr-1"></i>เพิ่มโบนัส</button>
            </form>
        </div>
    ...
    <form id="tierSettingsForm" class="p-6">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button type="button" id="addTierBtn" class="px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-colors"><i class="fas fa-plus mr-1"></i>เพิ่มระดับ</button>
            </div>
                ...
                <div class="tier-row bg-gray-50 rounded-lg p-4 border border-gray-200" data-index="<?= $index ?>">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div>
                            ...
                            <input type="text" name="tier_name[]" value="<?= htmlspecialchars($tier['name']) ?>" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" placeholder="เช่น Silver, Gold" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">คะแนนขั้นต่ำ</label>
                            <div class="flex items-center gap-2">
                                <input type="number" name="tier_points[]" value="<?= intval($tier['min_points']) ?>" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" min="0" step="100" placeholder="0" required>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <input type="number" name="tier_multiplier[]" value="<?= number_format(floatval($tier['multiplier']), 2) ?>" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" min="1" max="10" step="0.1" placeholder="1.0" required>
                                <span class="text-gray-500 text-sm">x</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <?php if ($index > 0): ?>
                            <button type="button" class="delete-tier-btn px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="ลบระดับนี้"><i class="fas fa-trash"></i></button>
                            <?php else: ?>
                            <span class="px-3 py-2 text-gray-400" title="ระดับแรกไม่สามารถลบได้"><i class="fas fa-lock"></i></span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php if ($index === 0): ?><p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle mr-1"></i>ระดับแรกคือระดับเริ่มต้นสำหรับสมาชิกใหม่ (คะแนนขั้นต่ำควรเป็น 0)</p><?php endif; ?>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            <button type="button" id="resetTiersBtn" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors"><i class="fas fa-undo mr-2"></i>รีเซ็ตเป็นค่าเริ่มต้น</button>
            <button type="submit" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors"><i class="fas fa-save mr-2"></i>บันทึกการตั้งค่า</button>
        </div>
    </form>
</div>
...
<div class="bg-white rounded-xl shadow mt-6">
    <div class="p-4 border-b"><h2 class="font-semibold text-gray-800"><i class="fas fa-chart-pie mr-2 text-green-500"></i>สรุประดับสมาชิกปัจจุบัน</h2></div>
    <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-<?= count($tierSettings) ?> gap-4">
            ...
            <div class="text-center p-4 rounded-lg border-2" style="border-color: <?= $color ?>; background: <?= $color ?>10;">
                <div class="w-12 h-12 mx-auto rounded-full flex items-center justify-center mb-2" style="background: <?= $color ?>;"><i class="fas fa-medal text-white text-xl"></i></div>
                <div class="font-bold text-lg" style="color: <?= $color ?>;"><?= htmlspecialchars($tier['name']) ?></div>
                <div class="text-sm text-gray-600 mt-1"><?= $index === 0 ? 'เริ่มต้น' : number_format($tier['min_points']) . '+ คะแนน' ?></div>
                <div class="text-xs text-gray-500 mt-1">ตัวคูณ: <?= number_format($tier['multiplier'], 1) ?>x</div>
                <?php if ($nextTier): ?><div class="mt-2 text-xs text-gray-400"><i class="fas fa-arrow-up mr-1"></i>อีก <?= number_format($nextTier['min_points'] - $tier['min_points']) ?> คะแนน</div>
                <?php else: ?><div class="mt-2 text-xs text-green-600"><i class="fas fa-crown mr-1"></i>ระดับสูงสุด</div><?php endif; ?>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
...
<div id="campaignModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div class="p-4 border-b flex items-center justify-between">
            ...
            <button type="button" onclick="closeCampaignModal()" class="p-2 hover:bg-gray-100 rounded-lg transition-colors"><i class="fas fa-times text-gray-500"></i></button>
        </div>
        <form id="campaignForm" class="p-4">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="campaignName" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" placeholder="เช่น Double Points Weekend">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">ตัวคูณแต้ม <span class="text-red-500">*</span></label>
                    <div class="flex items-center gap-2">
                        <input type="number" name="multiplier" id="campaignMultiplier" value="2.0" required min="1.1" max="10" step="0.1" class="w-24 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        <span class="text-gray-600">x แต้ม</span>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" id="campaignStartDate" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">วันสิ้นสุด <span class="text-red-500">*</span></label>
                        <input type="date" name="end_date" id="campaignEndDate" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    </div>
                </div>
                <div>
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="campaignIsActive" checked class="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
                        <span class="ml-2 text-gray-700">เปิดใช้งานแคมเปญ</span>
                    </label>
                </div>
            </div>
            <div class="mt-6 flex justify-end gap-3">
                <button type="button" onclick="closeCampaignModal()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">ยกเลิก</button>
                <button type="submit" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors"><i class="fas fa-save mr-1"></i>บันทึก</button>
            </div>
        </form>
    </div>
</div>
