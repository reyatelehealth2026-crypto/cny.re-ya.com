...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">📅 รายงานอัตโนมัติ</h1>
            <p class="text-gray-500 text-sm">ส่งรายงานสรุปทาง LINE ให้แอดมินอัตโนมัติ</p>
        </div>
        <button onclick="openCreateModal()" class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl p-12 text-center">
        ...
        <button onclick="openCreateModal()" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">
        ...
        <div class="report-card <?= $report['is_active'] ? '' : 'inactive' ?>">
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="openEditModal(<?= htmlspecialchars(json_encode($report)) ?>)" class="p-2 text-gray-500 hover:bg-gray-100 rounded-lg" title="แก้ไข">
    ...
    <div class="bg-white rounded-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <form method="POST" id="reportForm">
            ...
            <div class="p-6 border-b">
                <h3 class="text-lg font-semibold" id="modalTitle">สร้างรายงานอัตโนมัติ</h3>
            </div>
            
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-2">ชื่อรายงาน *</label>
                    <input type="text" name="name" id="inputName" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none" placeholder="เช่น สรุปยอดขายประจำวัน">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="schedule_type" id="inputScheduleType" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none" onchange="toggleDaySelect()">
                    ...
                    <div>
                        ...
                        <input type="time" name="schedule_time" id="inputScheduleTime" value="08:00" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none">
                    </div>
                </div>
                
                <div id="daySelectSection" class="hidden">
                    <label class="block text-sm font-medium mb-2">วันที่ส่ง</label>
                    <select name="schedule_day" id="inputScheduleDay" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none">
                ...
                <div>
                    ...
                    <div class="border rounded-lg p-3 max-h-40 overflow-y-auto space-y-2">
                        ...
                        <label class="flex items-center p-2 hover:bg-gray-50 rounded cursor-pointer">
                            <input type="checkbox" name="recipients[]" value="<?= $admin['id'] ?>" class="recipient-checkbox mr-3">
                            <div class="flex-1">
                ...
                <div id="activeSection" class="hidden">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="inputIsActive" value="1" checked class="w-5 h-5 text-green-500 rounded mr-3">
            ...
            <div class="p-6 border-t bg-gray-50 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">
                    <i class="fas fa-save mr-2"></i><span id="submitText">สร้างรายงาน</span>
                </button>
            </div>
        </form>
    </div>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitle').textContent = 'สร้างรายงานอัตโนมัติ';
    document.getElementById('submitText').textContent = 'สร้างรายงาน';
    document.getElementById('inputName').value = '';
    document.getElementById('inputScheduleType').value = 'daily';
    document.getElementById('inputScheduleTime').value = '08:00';
...
function openEditModal(report) {
    ...
    document.getElementById('modalTitle').textContent = 'แก้ไขรายงาน';
    document.getElementById('submitText').textContent = 'บันทึก';
    document.getElementById('inputName').value = report.name;
    document.getElementById('inputScheduleType').value = report.schedule_type;
    document.getElementById('inputScheduleTime').value = report.schedule_time.substring(0, 5);
    document.getElementById('inputScheduleDay').value = report.schedule_day || '1';
    document.getElementById('inputIsActive').checked = report.is_active == 1;
...
function toggleDaySelect() {
    ...
    const section = document.getElementById('daySelectSection');
    
    if (type === 'weekly' || type === 'monthly') {
        section.classList.remove('hidden');
        
        // Update options for monthly
        const select = document.getElementById('inputScheduleDay');
        if (type === 'monthly') {
            select.innerHTML = '';
            for (let i = 1; i <= 28; i++) {
