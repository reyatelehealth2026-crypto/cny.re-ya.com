        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4">
                    ...
                    <p class="text-xs text-gray-500">ใช้งาน <?php echo number_format($rule['usage_count'] ?? 0); ?> ครั้ง</p>
                </td>
                <td class="px-6 py-4">
                    <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-sm"><?php echo $rule['trigger_type']; ?></span>
                </td>
                <td class="px-6 py-4">
                    <span class="px-2 py-1 rounded text-sm" style="background-color: <?php echo $rule['tag_color'] ?? '#3B82F6'; ?>20; color: <?php echo $rule['tag_color'] ?? '#3B82F6'; ?>">
                ...
                <td class="px-6 py-4 text-sm text-gray-600">
                    ...
                    foreach ($conditions as $key => $value) {
                        switch ($key) {
                            case 'min_orders': $conditionText[] = "≥ {$value} orders"; break;
                            case 'max_orders': $conditionText[] = "≤ {$value} orders"; break;
                            case 'min_amount': $conditionText[] = "≥ ฿" . number_format($value); break;
                            case 'days': $conditionText[] = "{$value} วัน"; break;
                            case 'month': $conditionText[] = $value === 'current' ? 'เดือนปัจจุบัน' : "เดือน {$value}"; break;
                        }
                    }
                ...
                <td class="px-6 py-4 text-center">
                    <form method="POST" class="inline">
                        ...
                        <button type="submit" class="px-3 py-1 rounded-full text-xs <?php echo $rule['is_active'] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'; ?>">
        ...
        <form method="POST">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="rule_name" required class="w-full px-4 py-2 border rounded-lg" placeholder="เช่น ติด VIP เมื่อซื้อ 5 ครั้ง">
                </div>
                ...
                <div>
                    ...
                    <select name="trigger_type" id="triggerType" onchange="showConditionFields()" required class="w-full px-4 py-2 border rounded-lg">
                ...
                <div id="orderCountFields" class="hidden space-y-2">
                    <div>
                        ...
                        <input type="number" name="min_orders" min="1" class="w-full px-4 py-2 border rounded-lg" placeholder="เช่น 5">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">จำนวน Orders สูงสุด (ไม่บังคับ)</label>
                        <input type="number" name="max_orders" min="1" class="w-full px-4 py-2 border rounded-lg" placeholder="เช่น 10">
                    </div>
                </div>
                
                <div id="totalSpentFields" class="hidden">
                    <label class="block text-sm font-medium mb-1">ยอดซื้อขั้นต่ำ (บาท)</label>
                    <input type="number" name="min_amount" min="0" step="0.01" class="w-full px-4 py-2 border rounded-lg" placeholder="เช่น 10000">
                </div>
                
                <div id="inactivityFields" class="hidden">
                    <label class="block text-sm font-medium mb-1">จำนวนวันที่ไม่มีกิจกรรม</label>
                    <input type="number" name="inactive_days" min="1" class="w-full px-4 py-2 border rounded-lg" placeholder="เช่น 30">
                </div>
            ...
            <div class="p-6 border-t bg-gray-50 flex justify-end gap-2">
                ...
                <button type="submit" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">สร้าง Rule</button>
            </div>
        </form>
    </div>
</div>
